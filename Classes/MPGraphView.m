...
- (void)commonSetup
{
    self.buttonBounds = CGRectMake(0.0, 0.0, 0.0, 0.0);
    self.maxNumberOfButtons = 60;
}
...
- (UIBezierPath *)graphPathFromPoints{
    ...
    NSInteger step = ceilf(points.count/(CGFloat)self.maxNumberOfButtons);
    for (NSInteger i=0;i<points.count;i++) {
        ...
        if (i%step == 0)
        {
            ...
            button.frame=self.buttonBounds;
            button.center=point;
...
- (CGPoint)pointAtIndex:(NSInteger)index{
    ...
    CGFloat space=(self.contentWidth)/(points.count+1);
